...
struct PokemonWater: View {
    @ObservedObject var pokemonVM = PokemonViewModel() // Objeto que contém os dados e a lógica dos Pokémons
    
    // Filtra os Pokémons pelo seu tipo (água)
    var filteredPokemon: [Pokemon] {
            return pokemonVM.pokemon.filter { $0.type.lowercased() == "water" }
        }
    ...
    var body: some View {
        NavigationView {
            List {
                // Mostrar a lista dos Pokémons do tipo Água
...
struct PokemonWater_Previews: PreviewProvider {
