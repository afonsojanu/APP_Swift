...
struct PokemonElectric: View {
    @ObservedObject var pokemonVM = PokemonViewModel() // Objeto que contém os dados e a lógica dos Pokémons
    
    // Filtra os Pokémons pelo seu tipo (elétrico)
    var filteredPokemon: [Pokemon] {
            return pokemonVM.pokemon.filter { $0.type.lowercased() == "electric" }
        }
    ...
    var body: some View {
        NavigationView {
            List {
                // Mostrar a lista dos Pokémons do tipo Elétrico
...
struct PokemonElectric_Previews: PreviewProvider {
